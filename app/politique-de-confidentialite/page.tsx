import type { Metadata } from "next";

export const metadata: Metadata = {
  title: "Politique de confidentialité — Éliane Larre",
  description:
    "Politique de confidentialité — Éliane Larre, entraîneure personnelle privée à Montréal.",
  alternates: { canonical: "/politique-de-confidentialite" },
  openGraph: {
    type: "website",
    locale: "fr_CA",
    title: "Politique de confidentialité — Éliane Larre",
    description:
      "Politique de confidentialité — Éliane Larre, entraîneure personnelle privée à Montréal.",
    siteName: "Éliane Larre",
  },
  robots: { index: false },
};

export default function PolitiqueConfidentialitePage() {
  return (
    <main id="contenu-principal" className="legal-main">
      <section className="legal-section">
        <div className="legal-inner">
          <h1>Politique de confidentialité</h1>
          <p className="legal-updated">
            <em>Dernière mise à jour : 18 septembre 2026</em>
          </p>

          <p>
            La présente politique de confidentialité décrit la manière dont Éliane Larre (&laquo;&nbsp;je&nbsp;&raquo;,
            &laquo;&nbsp;mon&nbsp;&raquo;, &laquo;&nbsp;mes&nbsp;&raquo;) recueille, utilise, communique, conserve et
            protège les renseignements personnels que tu peux fournir lorsque tu visites ce site web ou communiques avec
            moi dans le cadre de mes services d&apos;entraînement personnel.
          </p>

          <p>
            Je m&apos;engage à protéger la confidentialité de tes renseignements personnels conformément aux lois
            applicables, notamment la Loi sur la protection des renseignements personnels dans le secteur privé (Québec)
            et la Loi 25.
          </p>

          <h2>1. Responsable de la protection des renseignements personnels</h2>
          <p>
            Éliane Larre est responsable de la protection des renseignements personnels recueillis dans le cadre de ses
            activités professionnelles.
          </p>
          <p>
            Pour toute question concernant la présente politique ou pour exercer tes droits en matière de protection des
            renseignements personnels, tu peux communiquer avec moi à :
          </p>
          <p>
            Courriel : <a href="mailto:info@elianelarre.com">info@elianelarre.com</a>
          </p>

          <h2>2. Renseignements personnels recueillis</h2>
          <p>Selon les fonctionnalités utilisées sur le site, je peux recueillir les renseignements suivants :</p>
          <ul>
            <li>Nom et prénom</li>
            <li>Adresse courriel</li>
            <li>Numéro de téléphone</li>
            <li>Informations fournies dans un formulaire de contact, de candidature ou de prise de rendez-vous</li>
            <li>
              Informations liées à tes objectifs, habitudes d&apos;entraînement ou besoins que tu choisis volontairement
              de partager
            </li>
            <li>
              Données techniques liées à l&apos;utilisation du site, telles que l&apos;adresse IP, le type
              d&apos;appareil, le navigateur utilisé et les pages consultées
            </li>
          </ul>
          <p>
            Je ne recueille que les renseignements nécessaires pour répondre à tes demandes, planifier un rendez-vous ou
            évaluer si mes services correspondent à tes besoins.
          </p>

          <h2>3. Utilisation des renseignements</h2>
          <p>Les renseignements recueillis peuvent être utilisés afin de :</p>
          <ul>
            <li>Répondre à tes demandes d&apos;information</li>
            <li>Communiquer avec toi concernant mes services</li>
            <li>Planifier et confirmer un appel découverte ou un rendez-vous</li>
            <li>T&apos;envoyer des messages texte lorsque tu y as consenti (voir la section 5)</li>
            <li>Évaluer si mon accompagnement correspond à tes objectifs et à ta situation</li>
            <li>Améliorer le contenu, les services et l&apos;expérience utilisateur du site</li>
            <li>Respecter mes obligations légales lorsque nécessaire</li>
          </ul>

          <h2>4. Consentement</h2>
          <p>
            En utilisant ce site ou en me transmettant volontairement des renseignements personnels, tu consens à leur
            collecte, à leur utilisation et à leur communication conformément à la présente politique.
          </p>
          <p>
            Tu peux retirer ton consentement en tout temps, sous réserve de certaines obligations légales pouvant
            s&apos;appliquer.
          </p>

          <h2>5. Messages texte</h2>
          <p>
            Si tu me fournis ton numéro de téléphone et que tu coches la case de consentement prévue à cet effet, je peux
            t&apos;envoyer des messages texte. Il y a deux types de messages, et chacun a sa propre case :
          </p>
          <ul>
            <li>
              <strong>Messages non promotionnels</strong>
              <br />
              Confirmations, rappels de rendez-vous et suivis liés à ton appel découverte et à ton accompagnement.
            </li>
            <li>
              <strong>Messages promotionnels</strong>
              <br />
              Offres spéciales, nouveautés et annonces concernant mes services. Cette case est toujours facultative : tu
              peux soumettre un formulaire sans la cocher.
            </li>
          </ul>
          <p>
            La fréquence des messages varie. Des frais de messagerie et de données peuvent s&apos;appliquer selon ton
            forfait.
          </p>
          <p>
            Tu peux te désabonner en tout temps en répondant STOP à l&apos;un de mes messages, ou obtenir de l&apos;aide
            en répondant HELP. Tu peux aussi m&apos;écrire à{" "}
            <a href="mailto:info@elianelarre.com">info@elianelarre.com</a>.
          </p>
          <p>
            Ton numéro de téléphone et ton consentement à recevoir des messages texte ne sont ni vendus ni communiqués à
            des tiers à des fins de marketing.
          </p>

          <h2>6. Partage des renseignements personnels</h2>
          <p>Je ne vends, ne loue ni n&apos;échange tes renseignements personnels.</p>
          <p>
            Je peux toutefois partager certains renseignements avec des fournisseurs de services technologiques lorsque
            cela est nécessaire au fonctionnement de mes activités, notamment pour :
          </p>
          <ul>
            <li>La prise de rendez-vous</li>
            <li>L&apos;hébergement du site web</li>
            <li>L&apos;analyse du trafic du site</li>
            <li>Les communications par courriel et par message texte</li>
          </ul>
          <p>
            Ces fournisseurs n&apos;ont accès qu&apos;aux renseignements nécessaires à l&apos;exécution de leurs services
            et sont tenus de protéger leur confidentialité conformément aux lois applicables.
          </p>
          <p>
            Je peux également communiquer certains renseignements lorsque la loi l&apos;exige ou lorsqu&apos;une autorité
            compétente en fait la demande.
          </p>

          <h2>7. Hébergement et transferts à l&apos;extérieur du Québec</h2>
          <p>
            Certains fournisseurs technologiques utilisés pour l&apos;exploitation du site peuvent héberger ou traiter
            des renseignements personnels à l&apos;extérieur du Québec ou du Canada.
          </p>
          <p>
            Lorsque cela est nécessaire, je prends des mesures raisonnables afin de m&apos;assurer que ces renseignements
            bénéficient d&apos;un niveau de protection adéquat.
          </p>

          <h2>8. Conservation des renseignements</h2>
          <p>
            Les renseignements personnels sont conservés uniquement pour la durée nécessaire aux fins pour lesquelles ils
            ont été recueillis ou pour respecter les obligations légales applicables.
          </p>
          <p>
            Lorsqu&apos;ils ne sont plus nécessaires, les renseignements sont supprimés, détruits ou anonymisés de façon
            sécuritaire.
          </p>

          <h2>9. Tes droits</h2>
          <p>Conformément aux lois applicables, tu peux notamment :</p>
          <ul>
            <li>Accéder aux renseignements personnels que je détiens à ton sujet</li>
            <li>Demander la rectification de renseignements inexacts ou incomplets</li>
            <li>Retirer ton consentement à certaines utilisations de tes renseignements</li>
            <li>Demander la suppression de renseignements lorsque la loi le permet</li>
            <li>Déposer une plainte auprès de la Commission d&apos;accès à l&apos;information du Québec</li>
          </ul>
          <p>
            Pour exercer ces droits, communique avec moi à :{" "}
            <a href="mailto:info@elianelarre.com">info@elianelarre.com</a>
          </p>

          <h2>10. Témoins de connexion (cookies)</h2>
          <p>
            Ce site utilise des témoins de connexion (&laquo;&nbsp;cookies&nbsp;&raquo;) afin d&apos;assurer son bon
            fonctionnement et d&apos;améliorer l&apos;expérience utilisateur.
          </p>
          <p>Les catégories de témoins pouvant être utilisées sont :</p>
          <ul>
            <li>
              <strong>Témoins nécessaires</strong>
              <br />
              Ces témoins sont essentiels au fonctionnement du site et ne peuvent être désactivés.
            </li>
            <li>
              <strong>Témoins d&apos;analyse</strong>
              <br />
              Ils permettent de comprendre comment les visiteurs utilisent le site afin d&apos;améliorer ses performances
              et son contenu. Ces témoins sont utilisés uniquement avec ton consentement lorsque requis.
            </li>
            <li>
              <strong>Témoins de marketing</strong>
              <br />À l&apos;heure actuelle, aucun témoin de marketing n&apos;est utilisé sur ce site. Si cela devait
              changer, ces témoins ne seraient activés qu&apos;avec ton consentement préalable.
            </li>
          </ul>
          <p>
            Tu peux modifier tes préférences en matière de témoins à tout moment à partir des paramètres prévus à cet
            effet sur le site.
          </p>

          <h2>11. Sécurité des renseignements</h2>
          <p>
            Je mets en place des mesures raisonnables de sécurité afin de protéger les renseignements personnels contre
            l&apos;accès non autorisé, la perte, le vol, la divulgation ou la modification.
          </p>
          <p>
            Malgré ces mesures, aucun système de transmission ou de stockage de données ne peut être garanti comme étant
            totalement sécurisé.
          </p>

          <h2>12. Liens vers des sites tiers</h2>
          <p>
            Le site peut contenir des liens vers des sites web ou services exploités par des tiers. Je ne suis pas
            responsable de leurs pratiques en matière de confidentialité et t&apos;invite à consulter leurs politiques
            respectives.
          </p>

          <h2>13. Modifications de la présente politique</h2>
          <p>
            Je me réserve le droit de modifier la présente politique de confidentialité à tout moment afin de refléter
            l&apos;évolution de mes pratiques, de mes services ou des exigences légales.
          </p>
          <p>
            La version la plus récente sera toujours disponible sur cette page et la date de mise à jour sera indiquée en
            haut du document.
          </p>

          <h2>14. Me joindre</h2>
          <p>
            Pour toute question concernant la présente politique de confidentialité ou le traitement de tes renseignements
            personnels :
          </p>
          <p>
            Éliane Larre
            <br />
            Courriel : <a href="mailto:info@elianelarre.com">info@elianelarre.com</a>
          </p>
        </div>
      </section>
    </main>
  );
}
